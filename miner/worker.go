...
type generateParams struct {
	...
	maxBlobCount *uint64           // Maximum number of blobs.
...
func (miner *Miner) prepareWork(genParams *generateParams, witness bool) (*environment, error) {
	...
	env, err := miner.makeEnv(parent, header, genParams.coinbase, witness, genParams.maxBlobCount)
	if err != nil {
...
func (miner *Miner) makeEnv(parent *types.Header, header *types.Header, coinbase common.Address, witness bool, maxBlobCount *uint64) (*environment, error) {
...
func (miner *Miner) commitBlobTransaction(env *environment, tx *types.Transaction) error {
	...
	maxBlobCount := uint64(params.MaxBlobGasPerBlock / params.BlobTxBlobGasPerBlob)
	if env.maxBlobCount != nil {
		maxBlobCount = *env.maxBlobCount
	}
	// Checking against blob gas limit: It's kind of ugly to perform this check here, but there
	// isn't really a better place right now. The blob gas limit is checked at block validation time
	// and not during execution. This means core.ApplyTransaction will not return an error if the
	// tx has too many blobs. So we have to explicitly check it here.
	if uint64(env.blobs+len(sc.Blobs)) > maxBlobCount {
...
func (miner *Miner) commitTransactions(env *environment, plainTxs, blobTxs *transactionsByPriceAndNonce, interrupt *atomic.Int32) error {
	...
	for {
		...
		if !blobTxs.Empty() && uint64(env.blobs) >= maxBlobCount {
