...
// GetTransactionBySenderAndNonce returns the hash of a transaction sent by
// the given account with the given nonce. The lookup checks the transaction
// pool first; on a miss it falls back to historical state for nodes that
// retain enough state history to answer the query. Returns nil if no such
...
func (api *TransactionAPI) GetTransactionBySenderAndNonce(ctx context.Context, sender common.Address, nonce hexutil.Uint64) (*common.Hash, error) {
	return api.b.GetTransactionBySenderAndNonce(ctx, sender, uint64(nonce))
}
