...
func TestSupplyGenesisAlloc(t *testing.T) {
	...
	out, err := testSupplyTracer(gspec, emptyBlockGenerationFunc)
	if err != nil {
...
func TestSupplyEip1559Burn(t *testing.T) {
	...
	out, err := testSupplyTracer(gspec, eip1559BlockGenerationFunc)
	if err != nil {
		t.Fatalf("failed to test supply tracer: %v", err)
	}
...
func TestSupplyWithdrawals(t *testing.T) {
	...
	out, err := testSupplyTracer(gspec, withdrawalsBlockGenerationFunc)
	if err != nil {
		t.Fatalf("failed to test supply tracer: %v", err)
	}
...
func testSupplyTracer(genesis *core.Genesis, gen func(*core.BlockGen)) ([]live.SupplyInfo, error) {
	var (
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create call tracer: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
	...
	if n, err := chain.InsertChain(blocks); err != nil {
		return nil, fmt.Errorf("block %d: failed to insert into chain: %v", n, err)
	}
