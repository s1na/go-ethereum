...
func (s *StateDB) fastDeleteStorage(snaps *snapshot.Tree, addrHash common.Hash, root common.Hash) (map[common.Hash][]byte, map[common.Hash][]byte, *trienode.NodeSet, error) {
	...
	stack := trie.NewStackTrie(func(path []byte, hash common.Hash, blob []byte) {
		nodes.AddNode(path, trienode.NewDeleted(common.CopyBytes(blob)))
	})
	for iter.Next() {
...
func (s *StateDB) slowDeleteStorage(addr common.Address, addrHash common.Hash, root common.Hash) (map[common.Hash][]byte, map[common.Hash][]byte, *trienode.NodeSet, error) {
	...
	for it.Next(true) {
		...
		nodes.AddNode(it.Path(), trienode.NewDeleted(common.CopyBytes(it.NodeBlob())))
	}
...
// CommitWithUpdate is the variant of Commit, returning the entire update as well.
func (s *StateDB) CommitWithUpdate(block uint64, deleteEmptyObjects bool, noStorageWiping bool) (*StateUpdate, error) {
	return s.commitAndFlush(block, deleteEmptyObjects, noStorageWiping)
}
